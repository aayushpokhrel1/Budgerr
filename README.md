...
**Next up (per §15.5): deploy — design approved, artifact prep starting (2026-07-16).** Full design in `docs/superpowers/specs/2026-07-16-deployment-design.md` (see §15.2 for the summary). Hardware **deferred (design for both)** because playstat's daily XGBoost retrain (~1M rows) is materially slower on a Pi 5 than the x86 laptop — owner picks the box after seeing real retrain time. playstat coordination is **done**: it supplied port (`uvicorn api.main:app :8000`), its own Postgres 14+ (~701 MB, schema `db/schema.sql`, migration `005` merges ~7/18), env (`DATABASE_URL`/`PLAYSTAT_API_KEYS`/`AUTH_ENABLED`/`API_BASKETBALL_KEY`/`ODDS_API_KEY`), its `mlb` daily chain, and it will author its own Dockerfile + auth-exempt `/health` at deploy time (Budgerr never edits the playstat repo). Deliverables this pass: `backend/Dockerfile`, `deploy/docker-compose.yml`, systemd `.timer`/`.service` units, `docs/DEPLOY.md`, and a small backend change exempting `/health` from the API-key guard (for the container healthcheck). **Prep-only; deployment is gated on the owner — nothing goes live.** Auth and encrypted backups are done — the two Section 10 non-negotiables are met.
