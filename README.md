...
- **CI — written, blocked on a token scope**: GitHub Actions workflows exist for all three repos (backend `pip install .[dev]` + pytest — DB-independent, no Postgres service needed; web `npm ci` + `npm run build`; mobile `npm ci` + `tsc --noEmit`). They're committed on an `add-ci-workflow` branch in each repo but **not pushed** — the git credential lacks the `workflow` OAuth scope, so GitHub rejects pushes that touch `.github/workflows/*`. To finish: `gh auth refresh -s workflow` (interactive), then in each repo `git checkout main && git merge --ff-only add-ci-workflow && git push`. Runs are unverified on GitHub's runners until then.
...
**Also shipped later this session (autonomous run):**
  ...
  - **CI workflows** — §15.2; written for all three repos but **not pushed** — the git token lacks the `workflow` OAuth scope. Parked on an `add-ci-workflow` branch in each repo. Finish with `gh auth refresh -s workflow` then merge+push each (see the CI bullet in §15.2). Also added `backend/tests/conftest.py` pinning auth off as the test baseline (the live `AUTH_ENABLED=true` in `.env` had started 401-ing the TestClient route tests).

**Next up (per §15.5):** **deploy** (VPS or Pi+Tailscale; move Budgerr + playstat together — auto-settle/auto-log assume playstat on the same host; launchd jobs become cron/systemd timers; Caddy for TLS if VPS). Auth and encrypted backups are done — the two Section 10 non-negotiables are now met.
