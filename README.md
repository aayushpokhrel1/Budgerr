...
- **Deploy** — **hardware chosen (2026-07-16): Raspberry Pi 5 (8GB), ARM64**, headless, Tailscale for phone access (no public HTTPS/VPS, so no Caddy/TLS needed for now — the tailnet is the transport). Move playstat and Budgerr **together** (auto-settle/auto-log assume playstat on the same host). Both stacks are already Docker-friendly (Postgres in compose); add multi-arch containers for the two APIs, launchd jobs become **systemd timers**, reuse `backend/ops/backup.sh` tooling to migrate the Postgres volume. Unlocks the phone outside home wifi. **In progress:** coordinating with the playstat session (owned separately, never modified here) for its Dockerfile/port/datastore/env so the combined compose can be authored in one shot; see §15.6.
...
**Next up (per §15.5): deploy — prep in progress (2026-07-16).** Hardware decided: **Raspberry Pi 5 8GB, ARM64**, headless + Tailscale (no VPS/public TLS, so no Caddy). Plan: multi-arch Dockerfiles for both APIs → one combined Docker Compose (Budgerr backend + Postgres + playstat) on the Pi; the four launchd jobs (`plaid-sync`/`auto-settle`/`auto-log-parlays`/`backup`) become **systemd timers**; migrate the Postgres volume with the existing `backend/ops/backup.sh`/`restore.md` tooling; repoint frontends' API URL at the Pi's tailnet name. Backend containerizes cleanly (`uvicorn app.main:app :8001`, `psycopg[binary]`, mounts `../static` so the build context must include repo-root `static/`). **Blocking the combined compose:** playstat (separate session, never modified here) must supply a Dockerfile + confirm port/datastore/env/own-jobs/healthcheck — a coordination request was sent to the "Playstat Architecture" session 2026-07-16; awaiting reply. This is a **prep-only** pass: author Dockerfiles + compose + systemd units + DEPLOY.md and commit them; no hardware is touched until the Pi is in hand. Auth and encrypted backups are done — the two Section 10 non-negotiables are met.
