...
- **Low-risk builder parlays in Tonight** — *IMPLEMENTED 2026-07-23 on branch `feat/builder-parlays` (both frontends); pending merge/push.* Spec + build notes: `docs/superpowers/specs/2026-07-22-low-risk-builder-parlays-design.md` (§9); plan: `docs/superpowers/plans/2026-07-22-low-risk-builder-parlays.md`. Both Tonight views now show the builder parlays (short list, top 4 by `joint_prob`, tier badge), retire the model parlay UI (orphaned `ParlayCard` + unused hooks deleted, `parlays.list`/types kept), and log inline as paper bets. Builder-leg games resolve from the builder RUN's own date (not the slate) so `placed_at`/matchups are correct; fully-past runs are hidden. Team (NRFI/F5) legs are log-only (no auto-settle) and were verified against a fixture; player + team logging verified end-to-end in the browser against real data (web has a new Vitest suite, 16 tests; mobile `tsc`-verified). Open refinement: ranking by `joint_prob` makes the 1.4x tier dominate (2.0x rarely shows). Playstat pivoted MLB from model-ranked player props to a market-ranked "low-risk parlay of the day" precomputed nightly and served fast/listable at `GET /parlay-builder/saved?limit=` (verified live through our proxy 2026-07-22: 200 in ~1.5s, both 1.4x/2.0x tiers). Budgerr surfaces it in Tonight and **retires the model `/parlay-recommendations` UI** (that source is being wound down — −49% over 54 paper bets; its contract stays intact/additive for now). Ranking + labels use de-vigged `joint_prob`/`market_prob`, **never `model_prob`** (context-only, often null). Both clients' `lib/playstat.ts` gain a `listBuilder` fetch + discriminated `PlaystatBuilderLeg` types (player | team, `player_id` nullable, `market ∈ {first_inning_runs,f5_runs}`). New `BuilderParlayCard` logs inline as a paper bet like the existing `ParlayCard`. **Team (NRFI/F5) legs price near coin-flip and effectively never clear Playstat's favorite floor, so real `saved` data is 100% player props today** — the team branch is built + verified against a fixture (matchup resolved via `game_id → /games`). **Team-market bets are log-only (no auto-settle)** — `BetLegInput` has no `game_id`/`market` and team-market settlement (`team_game_stats` keyed on game_id+market+side) stays on the documented HOLD. Follow-ups (not in this feature): repoint the 9am auto-log job off the wound-down model source; build team-market settlement when Playstat's team legs actually flow and the HOLD lifts.
- **Team-market tier (NRFI/F5) in Tonight** — *IMPLEMENTED 2026-07-28 on branch `feat/team-market-tier` (both frontends).* Spec: `docs/superpowers/specs/2026-07-28-team-market-tier-design.md`; plan: `docs/superpowers/plans/2026-07-28-team-market-tier.md`. Playstat's `GET /parlay-builder/saved` gained an additive `?tier=` param (`player` default / `team` NRFI-F5-only / `all` combined); the team tier is now populated with real data (verified live 2026-07-28: 20 rows, all `kind:"team"`, `joint_prob` ~0.31 — **explicitly not low-risk**). Budgerr now fetches the combined `?tier=all` feed ONCE (`listBuilder(100,'all')`) and partitions it client-side by `hasTeamLeg`: player constructions feed the existing "Low-risk builder parlays" section (output unchanged — the player partition of `?tier=all` is byte-equivalent to `?tier=player`), team constructions feed a new **"Team markets (NRFI/F5) — higher variance"** section. Each section resolves its games from its OWN run's date (the player run and team run are usually different days) and hides fully-past runs independently. The new section reuses the already-built team branch (`legDisplay`/`builderConstructionToBetInput`/`hasTeamLeg` in the unchanged, byte-identical `lib/builderParlays.ts`) via a new `BuilderParlayCard` `variant="variance"` (amber border + muted, non-green joint badge so higher-variance cards don't read as "safe"). The team tier is **empty by nature much of the time** — `[]` is a normal state, shown as a calm empty message, not an error. **Team bets stay log-only** (no auto-settle; backend team-market settlement still on HOLD). Verified end-to-end in the browser against real `?tier=team` data (via the dev-only `?demo=builder-team` reveal, repointed from its old fixture-injection role): amber team cards render with resolved matchups, and logging one produced a pending paper bet (cleaned up). Web `npm run build` + 18 Vitest; mobile `tsc` clean; `lib/builderParlays.ts` byte-identical across repos. Follow-up (not built): the leg now carries inline `home_team`/`away_team` — a future change could render matchups without the `/games` fetch (though `isRunFullyPast` still needs game status).
- **Migrated Tonight slate + quick-entry off the frozen model surfaces** — *IMPLEMENTED 2026-07-29 on branch `feat/off-frozen-surfaces` (both frontends).* Spec: `docs/superpowers/specs/2026-07-29-migrate-off-frozen-model-surfaces-design.md`; plan: `docs/superpowers/plans/2026-07-29-migrate-off-frozen-model-surfaces.md`. Playstat shelved the MLB model (their README §16, model −57% ROI) and **froze** `GET /edges`, `GET /game-predictions`, `GET /parlay-recommendations` — they keep serving last-computed rows but stop updating. Budgerr's runtime now avoids the frozen surfaces: the 9am `com.budgerr.auto-log-parlays` job was disabled 2026-07-28 (it pulled `/parlay-recommendations` + `/edges`), and this change moves the two remaining consumers onto the builder feed (`?tier=all`, already fetched on Tonight). The slate GameCards now show the builder's **player-prop legs** per game (`playerLegsByGame`, suppressing legs already shown in the "Low-risk builder parlays" section) and a **first-inning (NRFI) line** from the builder's team legs (`firstInningLegByGame`) instead of the frozen `/edges` chips + `/game-predictions` prob. The bet quick-entry "Tonight's edges" picker became **"Tonight's builder picks"** (`distinctPlayerLegs`, no suppression). New pure helpers live in the byte-identical `lib/builderParlays.ts` (`playerLegIdentity`, `playerLegKeys`, `distinctPlayerLegs`, `playerLegsByGame`, `firstInningLegByGame`; +5 Vitest). The `usePlaystatEdges`/`usePlaystatGamePredictions` hooks were deleted; the `playstatApi.edges`/`gamePredictions` methods + `PlaystatEdge`/`PlaystatGamePrediction` types are **kept** (contract intact). Coverage is intentionally sparse (the builder is a curated short list, not comprehensive like `/edges` was), so most slate games show nothing — normal. Verified end-to-end in the browser: **no `/edges` or `/game-predictions` network calls** fire on `/tonight`, suppression works, and quick-entry "+ Add to bet" pre-fills a correct leg. Web `npm run build` + 23 Vitest; mobile `tsc` clean; `lib/builderParlays.ts` byte-identical across repos.

- **Completed the bet-leg model (`game_id`/`player_id`/`market`)** — *IMPLEMENTED 2026-08-13 on branch `feat/leg-model-completion` (backend + both frontends).* Spec: `docs/superpowers/specs/2026-08-13-leg-model-completion-design.md`; plan: `docs/superpowers/plans/2026-08-13-leg-model-completion.md`. Dual-purpose plumbing for **closing-line value (CLV)** and **team-market settlement**, scoped by coordinating with the Playstat "Closing-line value investigation" session (2026-08-13). Playstat's verdict: build the *plumbing* now, keep any CLV *number* dark — there is **no closing line** (their last snapshot is ~100 min pre-pitch line *movement*, not a close), the signal is currently negative, and the read surface (a lookup endpoint vs. additive fields) is unfinalized (~3–4 week horizon; they'll ping). What shipped is only the join-key plumbing both features need: `bet_legs` gains three **nullable** columns — `game_id` (playstat id-space, no FK; both leg kinds), `player_id` (player legs), `market` (`first_inning_runs`/`f5_runs`, team legs) — via migration `7161fe789272` (additive, **no backfill**). `BetLegCreate`/`BetLegRead` (backend) and the `BetLeg`/`BetLegInput` TS types (both frontends) carry them; `create_bet`'s `BetLeg(**leg.model_dump())` persists them automatically. The byte-identical `lib/builderParlays.ts` `builderConstructionToBetInput` now populates `game_id`/`player_id` on player legs and `game_id`/`market` on team legs, and **stops folding** the market into `player_name`/`stat_type` for team legs (team `player_name` is now the clean matchup, `stat_type` null). Post-log leg display (`BetRow`/`RecentBetsList` web, `RecentBets` mobile) renders a new `betLegMarketLabel(leg.market) ?? leg.stat_type` — team legs show their `NRFI`/`F5` label; **player legs and legacy pre-migration team rows are unchanged** (market null → stat_type fallback), so no backfill needed and no regression. This gives builder legs the exact CLV join keys Playstat specified (player `(game_id, player_id, stat_type, line_value)`, team `(game_id, market, line_value)` + side) and **removes the structural blocker for team-market settlement** (which stays on its documented HOLD — no settlement wired here, team legs remain log-only; nulling team-leg `stat_type` keeps them excluded from auto-settle, verified by a new test). **No CLV read job, endpoint, or UI** — plumbing only; no `clv`/`value`/`edge`/`closing` naming (mirrors Playstat's "movement" convention). Verified: backend 78 pytest + migration applied + live API round-trip (player leg → `game_id`/`player_id`, team leg → `game_id`/`market`, `stat_type` null); web `npm run build` + 27 Vitest; mobile `tsc`; `lib/builderParlays.ts` byte-identical across repos; and end-to-end in the browser — logging a builder parlay on `/tonight` persisted real `game_id`/`player_id` on the legs and rendered correctly on `/bets` (test row cleaned up).
